...
- Remediation state: AWAITING_CI_AND_HUMAN_APPROVAL
- Risk tier: CRITICAL
...
- Candidate commit SHA: recorded after commit (round-10 final candidate;
  supersedes `d69cd81`)
...
- CI commit SHA and status: `PENDING` — chờ con người push branch (môi
  trường này không có GitHub credentials); CI phải xanh trên đúng candidate
  SHA trước release
