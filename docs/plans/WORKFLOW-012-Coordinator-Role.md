...
- Risk categories and escalation rationale: revision 1 lập luận giữ ELEVATED vì
  ...
  định vòng 2 chỉ ra trục lập luận đó sai: tier phải xét theo câu chữ của Risk
  Model rule 2, không theo hướng "siết hay nới". Planner bỏ lập luận cũ và nâng
  tier, thêm một lý do mà chính Planner chưa nêu ở các bản trước:
  **thay đổi này gỡ bỏ một điều khoản mà con người đã phê duyệt sáng cùng ngày**
  (`AGENTS.md` mục 7, merge 2026-07-27). Đảo ngược một quyết định quản trị đã
  được con người duyệt thì đáng đi đường nặng, bất kể chiều thay đổi. Cộng thêm
  Risk Model rule 7: Planner đã sai hai lần liên tiếp về phạm vi và mức rủi ro
  của chính plan này, nên "không chắc" là trạng thái đúng và phải lấy tier cao
  hơn. Hệ quả: cần **hai** independent reviewer, trong đó một adversarial.
...
- In scope: `AGENTS.md` (mục 2 và mục 7), `docs/handoffs/_TEMPLATE.md` (dòng 27
  mang cùng điều khoản kiêm nhiệm), và handoff của plan này.
...
**Phải nói rõ đọc hợp đồng nào** — thẩm định vòng 2 chỉ ra bản trước còn mơ hồ
đúng chỗ này, và mơ hồ thì quy tắc mất tác dụng. Quy định cụ thể: đọc hợp đồng
của **vai mình sắp hành động**; nếu đang soạn bảng phân vai thì đọc hợp đồng của
**vai mình tự điền cho chính mình**, vì đó là vai duy nhất mình sắp thực sự
nhận. Không bắt đọc hợp đồng của vai giao cho agent khác — điều đó vô ích và tốn
token.
...
Điều khoản này nằm ở **hai chỗ**, không phải một: `AGENTS.md` mục 7 và
`docs/handoffs/_TEMPLATE.md:27` ("an agent holding multiple roles must..."). Bản
revision 1 chỉ liệt kê chỗ thứ nhất — thẩm định vòng 2 bắt được thiếu sót này.
Sửa cả hai, nếu không thì mẫu biên bản vẫn dạy đúng thứ vừa bị cấm.
...
| Independent Reviewer (1)              | agy (`claude-opus-4-6-thinking`) | —              | CRITICAL cần hai reviewer tươi đọc từng dòng, khác Implementer.                                                                                                                   | chưa               |
| Independent Reviewer (2, adversarial) | **chưa chọn**                    | —              | CRITICAL bắt buộc một vòng adversarial; phải khác cả Implementer lẫn reviewer 1.                                                                                                  | chưa               |
...
- [ ] `docs/handoffs/_TEMPLATE.md` không còn câu cho phép một execution giữ
      nhiều vai; Role execution log vẫn giữ đủ bốn dòng vai.
- [ ] Không thêm vai mới, không sửa file kiến trúc, không sửa hợp đồng vai nào.
- [ ] Hai vòng independent review đã đóng, trong đó một adversarial (yêu cầu của
      tier CRITICAL).
