...
- **`a5-l1-c6`** (Nâng cao: Hoá trị, số oxi hoá và công thức oxit) —
  **CẦN SỬA nhẹ**. Nội dung hiện tại: *"Nếu M chỉ có một số oxi hoá k
  thì x.k = 2y..."*. Lý do: quy tắc x·k=2y đúng với mọi oxit MxOy bất
...
- **`a5-l4-c6`** (Nâng cao: Bảo toàn oxi và lượng H+ của axit) —
  ...
  *"Nếu hỗn hợp chứa a mol MxOy và b mol RpSq thì nO = ay + bq."*. Lý
...
- **`a5-l4-c8`** (Nâng cao: Axit thiếu và giới hạn của dữ kiện) —
  **CẦN SỬA**. Nội dung hiện tại (đã đối chiếu đúng với file thật):
  *"...hỗn hợp MgO và CuO gặp 0,20 mol HCl chỉ cho biết tổng số mol
  oxit **đơn chức** đã phản ứng là 0,10 mol..."*. Lý do: "đơn chức" là
...
- **`a6-l1-c1`** (Axit là gì?, bổ sung) — **CẦN SỬA nhẹ**. Câu *"...
  không thể kết luận dung dịch nào có axit mạnh hơn chỉ từ pH nếu
  chưa biết nồng độ"* lẫn lộn "lực axit" (đặc tính của chất, ví dụ
...
- **a8-l4** (11 thẻ `c1..c11`): đa số ĐÚNG. 1 finding CẦN SỬA (lỗi
  ...
  - **`a8-l4-c4`** (Nâng cao: Hàm lượng dinh dưỡng của phân đạm) —
    **CẦN SỬA**. Nội dung hiện tại: *"NH4Cl có 28/53,5 x 100% =
    26,17% N"*. NH4Cl chỉ có 1 nguyên tử N (M=14), không phải 2 (M
...
- **a10-l1** (10 thẻ `c2..c11`): đa số ĐÚNG. 2 finding CẦN SỬA:
  ...
  - **`a10-l1-c11`** (Nâng cao: Hỗn hợp muối và chiến lược giải bài
    ...
    *"Không được tự chọn một ion 'phản ứng trước' chỉ vì kim loại của
    nó đứng thấp hơn trong dãy nếu đề không cho cơ sở cho lựa chọn
    ấy"* **mâu thuẫn trực tiếp** với `a10-l4-c2` (cùng unit, bài sau)
    vốn khẳng định: *"M sẽ phản ứng lần lượt với muối của kim loại có
    tính oxi hoá mạnh hơn trước... 'phản ứng trước' là quy ước biện
    luận"* — tức xác nhận thứ tự ưu tiên theo dãy hoạt động LÀ quy
...
- **a11-l2** (8 thẻ `c4..c11`): đa số ĐÚNG. 1 finding CẦN SỬA (thuật
  ...
  - **`a11-l2-c4`** (Nâng cao: Clo trong nước và sự tẩy màu) — **CẦN
    SỬA nhẹ**. Nội dung hiện tại: *"đây là phản ứng tự oxi hoá - khử
    (phân hủy không đều)"*. Cụm "phân hủy không đều" không phải thuật
...
dùng đúng 22,4). Không sót tiếng Anh (trừ *cis/trans* là thuật ngữ
...
- **`b5-l3-c2`** (Dạng câu ghép nhiều phát biểu) — **SAI, CẦN SỬA
  (lỗi hoá học thật)**. Nội dung hiện tại: *"'PVC được trùng hợp trực
  ...
  trước khi trùng hợp."* Vế "hoặc C2H4" **sai**: C2H4 + HCl chỉ tạo
